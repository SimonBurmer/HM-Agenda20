...
public class TaskServiceImpl implements TaskService {
  ...
  public Long createTask(String uuid, String title, String login, String taskShortDescription,
    ...
    if (optTopic.isPresent()) {
      ...
      if (imageFile == null || imageFile.isEmpty()) {
        ...
        try {
          Task task =
              new Task(t, title, taskShortDescription, taskLongDescription, imageFile.getBytes());
          taskRepository.save(task);
          return task.getId();
        } catch (IOException e) {
  ...
  public List<SubscriberTaskDto> getTasksForTopic(String uuid, String login) {
    ...
    LOG.debug("Fordere Liste aller Tasks für Topic mit UUID {} und Anwender \"{}\" an.", uuid,
        login);
    User user = anwenderRepository.getOne(login);
