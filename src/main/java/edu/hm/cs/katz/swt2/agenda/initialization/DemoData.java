...
public class DemoData {
  ...
  private static final String TOPIC_HTML_LANG =
      "In diesem Tutorial lernen Sie die Grundlagen von HTML kennen. Und keine Sorge, diese vier Buchstaben dürfen Sie gerne deutsch aussprechen. Wofür diese Buchstaben im Einzelnen stehen, verraten wir Ihnen auf jeden Fall noch. Jedenfalls sind Sie herzlich eingeladen, mit uns eine Reise durch die Techniken des Internets zu wagen. Zunächst: Sie brauchen keine Internetverbindung, um mit HTML Webseiten erstellen und testen zu können. Solche Webseiten bestehen nämlich, wie Sie feststellen werden, aus ganz normalen Dateien, die Sie einfach auf Ihrem Computer speichern können. Um HTML-Dateien auf dem Bildschirm anzeigen zu können, benötigen Sie dann nur Ihren Browser, also das Programm, mit dem Sie gerade dieses Tutorial lesen. Die HTML-Dateien (und auch CSS-Stylesheets ab Kapitel 7), um die es im Folgenden hauptsächlich geht, sind reine Textdateien. Um diese Dateien bearbeiten zu können, reicht erstmal ein einfacher Texteditor, der bei Ihrem Betriebssystem bereits vorinstalliert ist. Beispiele: •   Linux: KWrite, gedit •   (Mac) OS X: TextEdit •   Windows: Editor/Notepad. Es gibt aber eine Vielzahl von oft auch kostenlosen Code-Editoren.";

  private static final String TOPIC_CSS_LANG =
      "Das World Wide Web Konsortium (W3C) entwickelte CSS, um zum ursprünglichen Grundgedanken von HTML zurückzukehren: Die Trennung der Informationen von der Präsentation. Ohne CSS ist HTML für Inhalt, Struktur und Aussehen zuständig. Ein aufgeblähter und unübersichtlicher Code entsteht. Durch die Nutzung von CSS, ist HTML für die Struktur eines Dokumentes zuständig - CSS dagegen ist für das Aussehen (Design) verantwortlich. CSS wird wie schon HTML als reiner Text \"geschrieben\". Wir erstellen also nur eine Textdatei, in der das Design definiert wird.";

  private static final String TOPIC_BACKKURS_KURZ =
      "Bei einem Ernies-Backkurs lernen Sie, wie Sie die hauchzarten und bildschönen französischen Leckerbissen perfekt formen und füllen.";
  ...
  public void addData() {
    ...
    Long affenMuffinTask = taskService.createTask(erniesKursUuid, "Affenmuffins backen",
        LOGIN_ERNIE, TASK_AFFENMUFFINS_BACKEN_KURZ, TASK_AFFENMUFFINS_BACKEN_LANG, TaskTypeEnum.OPTIONAL);
